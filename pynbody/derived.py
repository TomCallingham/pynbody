...
def vtheta(self):
    ...
    return (
        np.cos(self["az"]) * np.cos(self["theta"]) * self["vx"]
...
def _mu_from_temperature_threshold(sim, Y, t0):
    warnings.warn(
        "No ionization fractions found, assuming fully ionised gas above 10^4 and neutral below 10^4K. "
        "This is a very crude approximation."
    )
    x = np.empty(len(sim)).view(array.SimArray)
    if t0 is None:
        t0 = sim["temp"]
    x[np.where(t0 >= 1e4)[0]] = 4.0 / (8 - 5 * Y)
    x[np.where(t0 < 1e4)[0]] = 4.0 / (4 - 3 * Y)
    return x
...
def _mu_from_electron_frac(sim, Y):
    return 4.0 / (4.0 - 3.0 * Y + 4 * (1.0 - Y) * sim["ElectronAbundance"])


@SimSnap.derived_array
def p(sim):
    """Pressure"""
    p = sim["u"] * sim["rho"] * (2.0 / 3)
    p.sim = sim
    p.convert_units("Pa")
    return p
...
def zeldovich_offset(self):
    ...
    bdot_by_b = analysis.cosmology.rate_linear_growth(
        self, unit="km Mpc^-1 s^-1"
    ) / analysis.cosmology.linear_growth_factor(self)
...
def tform(self):
    ...
    t = analysis.cosmology.age(self, 1.0 / self["aform"] - 1.0)
    return t
