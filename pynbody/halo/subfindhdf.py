...
class SubFindHDFHaloCatalogue(HaloCatalogue):
    ...
    def __init__(self, sim, filename=None, subs=None, subhalos=False, _inherit_data_from=None):
        ...
        if subs is not None:
            warnings.warn(
                "The 'subs' argument to SubFindHDFHaloCatalogue is deprecated. Use 'subhalos' instead.",
                DeprecationWarning,
            )
            subhalos = subs
        ...
        if _inherit_data_from:
            ...
            self._init_subhalo_offset_data()
            # Now lazy
    ...
    def __inherit_data(self, parent):
        attrs_to_share = [
            ...
            "_subfind_halo_lengths",
            "fof_ignore",
            "sub_ignore",
            "_subfind_halo_parent_groups",
            "_ngroups",
            "_nsubhalos",
        ]
    ...
    @property
    def _fof_properties(self):
        if hasattr(self, "__fof_properties"):
            return self.__fof_properties
        self.__init_halo_properties()
        return self.__fof_properties

    @property
    def _sub_properties(self):
        if hasattr(self, "__sub_properties"):
            return self.__sub_properties
        self.__init_halo_properties()
        return self.__sub_properties

    def __init_halo_properties(self):
        self.__init_ignorable_keys()
        self.__fof_properties = self.__get_property_dictionary_from_hdf(self._fof_name)
        self.__sub_properties = self.__get_property_dictionary_from_hdf(self._subfind_name)
    ...
    def __reshape_multidimensional_properties(self):
        self.__reshape_multidimensional_properties_one_dictionary(self.__sub_properties, self._nsubhalos)
        self.__reshape_multidimensional_properties_one_dictionary(self.__fof_properties, self._ngroups)
    ...
    def __reassign_properties_from_sub_to_fof(self):
        reassign = []
        for k, v in self.__sub_properties.items():
            if v.shape[0] == self._ngroups:
                reassign.append(k)

        for reassign_i in reassign:
            self.__fof_properties[reassign_i] = self.__sub_properties[reassign_i]
            del self.__sub_properties[reassign_i]
        ...
        for fam in self.base._families_ordered():
            ...
            for ptype in ptypes:
                ...
                for h in self._hdf_files:
                    length = self._get_halodata_array_with_default(
                        h, self._grp_len_name, self._fof_name, ptype, np.array([])
                    )
                    ...
                    offset = self._get_halodata_array_with_default(
                        h, self._grp_offset_name, self._fof_name, ptype, None
                    )
    ...
    def _init_subhalo_offset_data(self):
        self._subfind_halo_offsets = {}
        self._subfind_halo_lengths = {}

        first_subs_in_groups = self._fof_group_first_subhalo.copy()
        first_subs_in_groups = first_subs_in_groups[
            (first_subs_in_groups > -1) & (first_subs_in_groups < self._nsubhalos)
        ]  # Just need actual first subhalo numbers, don't need to worry about haloes with no subhaloes. Accounts for formats where *no* first subhalo is recorded as i) '-1' and ii) 'total number of subhaloes'
        ...
        for fam in self.base._families_ordered():
            ...
            for ptype in ptypes:
                ...
                for h in self._hdf_files:
                    length = self._get_halodata_array_with_default(
                        h, self._sub_len_name, self._subfind_name, ptype, np.array([])
                    )
                    self._subfind_halo_lengths[ptype][curr_subhalos : curr_subhalos + len(length)] = length.astype(
                        "int64"
                    )[:]
                    curr_subhalos += len(length)

                lengths = self._subfind_halo_lengths[ptype][: self._nsubhalos]
                g = np.empty(self._nsubhalos + 1, dtype=float)
                g[0] = 0.0
                np.cumsum(lengths, out=g[1:])

                # For each subhalo index, determine which halo group it belongs to.
                # For a sorted array first_subs_in_groups, the group index for subhalo i is:
                #
                # get the index of the group_id of subhalo
                group_idx = (
                    np.searchsorted(first_subs_in_groups, np.arange(self._nsubhalos), side="right") - 1
                )  # The group the subhalo belongs too

                group_start = first_subs_in_groups[group_idx]

                parent_groups = self._subfind_halo_parent_groups[group_start]
                parent_fof_offset = self._fof_group_offsets[ptype][parent_groups]

                offsets = parent_fof_offset + (g[:-1] - g[group_start])
                self._subfind_halo_offsets[ptype][: self._nsubhalos] = offsets.astype("int64")
    ...
    def _write_pynbody_index_list_into_array(
        self, particle_indices, halo_or_group_offsets, halo_or_group_lengths, halo_or_group_index, type_map
    ):
        npart = 0
...
class Gadget4SubfindHDFCatalogue(SubFindHDFHaloCatalogue):
    ...
    def _get_progenitor_or_descendant_multifile(cls, sim, prog_or_desc, user_provided_filename):
        ...
        return Gadget4SubfindHdfProgenitorsMultiFileManager(
            cls._catalogue_filename(sim, user_provided_filename, "subhalo_" + prog_or_desc + "_")
        )
