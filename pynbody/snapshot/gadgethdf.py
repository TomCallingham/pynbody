...
class _GadgetHdfMultiFileManager:
    ...
    def __init__(self, filename, mode="r"):
        ...
        if h5py.is_hdf5(filename):
            ...
            self._filenames = [
                self._make_filename_for_cpu(filename, i) for i in range(self._numfiles)
            ]
...
class _HDFArrayFiller:
    ...
    def __init__(self, sim_array_to_fill=None, hdf_dataset=None):
        ...
        self.sim_element_size = (
            1
            if sim_array_to_fill is None
            else self._get_element_size(sim_array_to_fill)
        )
        self.file_element_size = (
            1 if hdf_dataset is None else self._get_element_size(hdf_dataset)
        )
    ...
    def fill_array_from_hdf_dataset(
        self,
        sim_array_to_fill,
        hdf_dataset,
        source_sel: slice | np.ndarray | None,
        offset: int = 0,
    ):
        ...
        if isinstance(source_sel, np.ndarray):
            ...
            raise TypeError(
                f"Unsupported source_sel type: {type(source_sel)}. "
    ...
    def _preprocess_source_selection(self, source_sel, offset):
        ...
        if isinstance(source_sel, slice):
            ...
            if (
                len(source_sel) > 1
                and source_sel[-1] - source_sel[0] == len(source_sel) - 1
            ):
    ...
    def _fill_from_fancy_index(self, sim_array_to_fill, hdf_dataset, source_sel):
        ...
        data_chunk_from_hdf = data_chunk_from_hdf.reshape(
            num_read, *sim_array_to_fill.shape[1:]
        )
    ...
    def _fill_from_slice(self, sim_array_to_fill, hdf_dataset, source_sel):
        ...
        if self.need_rescale:
            source_sel = self._get_contiguous_hdf_slice(
                source_sel.start, source_sel.stop - 1
            )
    ...
    def _get_contiguous_hdf_slice(self, id_min, id_max):
        ...
        if self.need_rescale:
            return np.s_[
                int(id_min * self.scaling_factor) : int(
                    (id_max + 1) * self.scaling_factor
                )
            ]
        else:
...
class HDFArrayLoader:
    ...
    def __init__(
        ...
        all_families: list[family.Family],
        family_to_group_map: dict[family.Family, list[str]],
    ...
    def __init_file_map(self):
        ...
        for fam in self._all_families:
            ...
            for particle_type in self._family_to_group_map[fam]:
                ...
                for hdf_group in self._hdf_files.iter_particle_groups_with_name(
                    particle_type
                ):
                    ptype_slice_len += hdf_group[
                        self._hdf_files._size_from_hdf5_key
                    ].size
                    self._file_interrupt_points[particle_type].append(ptype_slice_len)
                self._file_ptype_slice[particle_type] = slice(
                    ptype_slice_start, ptype_slice_start + ptype_slice_len
                )
                family_length += ptype_slice_len
                ptype_slice_start += ptype_slice_len

            self._file_family_slice[fam] = slice(
                family_slice_start, family_slice_start + family_length
            )
    ...
    def __init_load_map(self, take=None):
        ...
        self._load_control = chunk.LoadControl(
            self._file_ptype_slice, _max_buf, take
        )  # use HDF groups type instead of family type here
        ...
        for fam in self._all_families:
            ...
            for particle_type in self._family_to_group_map[fam]:
                ptype_slice_len = (
                    self._load_control.mem_family_slice[particle_type].stop
                    - self._load_control.mem_family_slice[particle_type].start
                )

                family_length += ptype_slice_len
                ptype_slice_start += ptype_slice_len

            self._family_slice_to_load[fam] = slice(
                family_slice_start, family_slice_start + family_length
            )
            family_slice_start += family_length
    ...
    def load_arrays(
        self,
        all_fams_to_load: list[family.Family],
        sim: SimSnap,
        array_name: str,
        ...
        for loading_fam in all_fams_to_load:
            sim_fam_array, array_filler = self._get_array_filler(
            ...
            for hdf_group_name in self._family_to_group_map[loading_fam]:
                if (
                    self._file_ptype_slice[hdf_group_name].stop
                    <= self._file_ptype_slice[hdf_group_name].start
                ):
                    continue
                # Create iterator for this group type across all files
                file_iterator = _HDFFileIterator(
                    iter(self._hdf_files.iter_particle_groups_with_name(hdf_group_name))
                )
                ...
                for (
                    ...
                    buf_index,
                    mem_index,
                ) in self._load_control.iterate_with_interrupts(
                    hdf_group_name,
                    hdf_group_name,
                    ...
                    if last_file_index != file_iterator.file_index:
                        dataset = self._get_dataset_from_translated_names(
                            sim, file_iterator.current_hdf_file, translated_names
                        )
                    ...
                    if dataset is not None:
                        ...
                        array_filler.fill_array_from_hdf_dataset(
                            target_array,
                            dataset,
                            source_sel=buf_index,
                            offset=file_iterator.particle_offset,
                        )
                    file_iterator.particle_offset += readlen
                    i0 = i1

    def _get_array_filler(
        self,
        array_name: str,
        loading_fam: family.Family,
        sim: SimSnap,
        translated_names: list[str],
    ):
        """
        ...
        for hdf_group_name in self._family_to_group_map[loading_fam]:
            ...
            for hdf_group in self._hdf_files.iter_particle_groups_with_name(
                ...
                first_dataset = self._get_dataset_from_translated_names(
                    sim, hdf_group, translated_names
                )
    ...
    def _get_dataset_from_translated_names(
        self, sim_obj, hdf_particle_group, translated_names
    ):
...
class GadgetHDFSnap(SimSnap):
    ...
    def _remove_empty_particle_groups(self):
        ...
        for family_name in self._family_to_group_map:
            self._family_to_group_map[family_name] = [
                group_name
                for group_name in self._family_to_group_map[family_name]
                if self._gadget_ptype_slice[group_name].stop
                > self._gadget_ptype_slice[group_name].start
            ]
    ...
    def _get_all_particle_arrays(self, gtype):
        ...
        l = [
            item
            for sublist in [
                self._get_hdf_allarray_keys(x[gtype]) for x in self._hdf_files
            ]
            for item in sublist
        ]
    ...
    def _get_or_create_hdf_dataset(self, particle_group, hdf_name, shape, dtype):
        if (
            self._translate_array_name(hdf_name, reverse=True)
            == self._mass_pynbody_name
        ):
    ...
    def _get_hdf_dataset(self, particle_group, hdf_name):
        ...
        if (
            ...
            try:
                ...
                if mtab > 0:
                    return _DummyHDFData(
                        mtab,
                        particle_group[self._size_from_hdf5_key].size,
                        self._mass_dtype,
                    )
            except (IndexError, KeyError):
                pass
        elif (
            self._translate_array_name(hdf_name, reverse=True) == self._eps_pynbody_name
        ):
            eps = self._get_softening_for_particle_group(particle_group)
            if eps is not None:
                return _DummyHDFData(
                    eps, particle_group[self._size_from_hdf5_key].size, self._mass_dtype
                )
    ...
    def _get_cosmo_factors(cls, hdf, arr_names):
        ...
        matching_hdf_keys = lambda arr_name: [
            s
            for s in GadgetHDFSnap._get_hdf_allarray_keys(hdf)
        ...
        if len(match) > 0:
            ...
            if "aexp-scale-exponent" in attrs:
                ...
                raise KeyError(
                    "Unable to find aexp-scale-exponent or a_scaling attribute for array %s"
                    % arr_name
                )
            ...
            if "h-scale-exponent" in attrs:
                ...
                raise KeyError(
                    "Unable to find h-scale-exponent or h_scaling attribute for array %s"
                    % arr_name
                )

            return units.a ** util.fractions.Fraction.from_float(
                float(aexp)
            ).limit_denominator(), units.h ** util.fractions.Fraction.from_float(
                float(hexp)
            ).limit_denominator()
        else:
    ...
    def _get_units_from_hdf_attr(self, hdfattrs):
        ...
        arr_units = self._get_units_from_description(
            VarDescription, CGSConversionFactor
        )
        if not np.allclose(aexp, 0.0):
            arr_units *= (units.a) ** util.fractions.Fraction.from_float(
                float(aexp)
            ).limit_denominator()
        if not np.allclose(hexp, 0.0):
            arr_units *= (units.h) ** util.fractions.Fraction.from_float(
                float(hexp)
            ).limit_denominator()
        return arr_units

    def _get_units_from_description(
        self, description, expectedCgsConversionFactor=None
    ):
            ...
            if unitname in description:
                ...
                if not np.allclose(power, 0.0):
                    arr_units *= (
                        ...
                        ** util.fractions.Fraction.from_float(
                            float(power)
                        ).limit_denominator()
                    )
                if not np.allclose(power, 0.0):
    ...
    def _load_array(self, array_name, fam=None):
        if not self._family_has_loadable_array(fam, array_name):
            ...
            self._array_loader.load_arrays(
                all_fams_to_load, self, array_name, translated_names
            )
    ...
    def __get_dtype_dims_and_units(self, fam, translated_names):
        ...
        for hdf0 in self._hdf_files:
            ...
            if hasattr(representative_dset, "attrs"):
                inferred_units = self._get_units_from_hdf_attr(
                    representative_dset.attrs
                )
    ...
    def _init_unit_information(self):
        ...
        if self._velocity_unit_key not in atr.keys():
            warnings.warn(
                "No unit information found in GadgetHDF file. Using gadget default units.",
                RuntimeWarning,
            )
            ...
            self._file_units_system = [
                units.Unit(x) for x in [vel_unit, dist_unit, mass_unit, "K"]
            ]
            return
        ...
        if cosmo:
            try:
                ...
                if self._units_need_hubble_factors:
                    ...
                    dist_unit *= units.a
                warnings.warn(
                    "Unable to find cosmological factors in HDF file; assuming position is %s"
                    % dist_unit
                )
            try:
                ...
                warnings.warn(
                    "Unable to find cosmological factors in HDF file; assuming velocity is %s"
                    % vel_unit
                )
            try:
                for fac in self._get_cosmo_factors(
                    self._hdf_files[0], ("Mass", "Masses")
                ):
                    mass_unit *= fac
            except KeyError:
                if self._units_need_hubble_factors:
                    mass_unit *= units.h**-1
                warnings.warn(
                    "Unable to find cosmological factors in HDF file; assuming mass is %s"
                    % mass_unit
                )
    ...
    def _init_properties(self):
        ...
        if "BoxSize" in atr:
            self.properties["boxsize"] = atr["BoxSize"] * self.infer_original_units(
                "cm"
            )
            ...
            if self._param_filename is not None:
                if not os.path.exists(self._param_filename):
                    raise FileNotFoundError(
                        f"Parameter file not found: {self._param_filename}"
                    )

                keys = [
                    self._param_file_velocity_unit_key,
                    self._param_file_length_unit_key,
                    self._param_file_mass_unit_key,
                ]
                atr = self._get_gizmo_param_values(keys)
                atr[self._velocity_unit_key] = units.Unit(
                    atr[self._param_file_velocity_unit_key]
                )
                atr[self._length_unit_key] = units.Unit(
                    atr[self._param_file_length_unit_key]
                )
                atr[self._mass_unit_key] = units.Unit(
                    atr[self._param_file_mass_unit_key]
                )
                self._units_need_hubble_factors = True

        if self._velocity_unit_key not in atr.keys():
            warnings.warn(
                "No unit information found either in the HDF file itself, or as a Gizmo parameter file. Units will revert to defaults. To use a gizmo paramfile, provide its full path as 'param_filename'",
                RuntimeWarning,
            )
        ...
        *subs* should be an integer specifying the parent FoF number"""
        ...
        if subs:
            ...
            cat = halo.number_array.HaloNumberCatalogue(
                parent_group,
                array="SubGroupNumber",
                ignore=np.max(self["SubGroupNumber"]),
            )
            cat._keep_subsnap_alive = parent_group  # by default, HaloCatalogue only keeps a weakref (should this be changed?)
            return cat
        else:
            return halo.number_array.HaloNumberCatalogue(
                self, array="GroupNumber", ignore=np.max(self["GroupNumber"])
            )
...
def p(sim):
    ...
    p[oneos] = (
        critpres * (sim.g["rho"][oneos].in_units("m_p cm**-3") / critdens) ** gammaeff
    )
...
def doppler_redshift(sim):
    ...
    return (
        np.sqrt(
            (1.0 + sim["losvel"].in_units("c")) / (1.0 - sim["losvel"].in_units("c"))
        )
        - 1.0
    )
