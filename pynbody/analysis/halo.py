...
def virial_radius(sim, cen=None, overden=178, r_max=None, rho_def="matter"):
    ...
    with transform:
        ...
        result = util.bisect(r_min, r_max, lambda r: target_rho - rho(r), epsilon=0, eta=1.0e-3 * target_rho)
...
def shape(sim, nbins=100, rmin=None, rmax=None, bins="equal", ndim=3, max_iterations=10, tol=1e-3, justify=False):
    ...
    # -----------------------------FUNCTIONS-----------------------------
    sn = lambda r, N: np.append([r[i * int(len(r) / N) : (1 + i) * int(len(r) / N)][0] for i in range(N)], r[-1])
    ...
    for i in range(nbins):
        ...
        iteration_counter = 0
        while (
            (np.abs(a[1] / a[0] - np.sort(a2)[-2] / max(a2)) > tol) & (np.abs(a[-1] / a[0] - min(a2) / max(a2)) > tol)
